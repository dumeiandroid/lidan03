...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === 'POST' && action === 'login') {
      ...
      await createTableIfNotExists(DB_GBKP, targetTable);

      const user = await DB_GBKP.prepare(
    ...
    if (method === 'GET' && action === 'list_tables') {
      const { results } = await DB_GBKP.prepare(
    ...
    if ((method === 'POST' || method === 'GET') && action !== 'login' && action !== 'list_tables') {
      await createTableIfNotExists(DB_GBKP, tableName);
    }

    switch (method) {
      case 'GET':
        if (id) {
          return await getContactById(DB_GBKP, tableName, id, corsHeaders);
        }
        return await getFilteredContacts(request, DB_GBKP, tableName, corsHeaders);

      case 'POST':
        return await createContact(request, DB_GBKP, tableName, corsHeaders);

      case 'PUT':
        if (!id) throw new Error('id_x required for PUT.');
        return await updateContact(request, DB_GBKP, tableName, id, corsHeaders);

      case 'DELETE':
        // --- DROP TABLE (reset id_x) ---
        if (action === 'drop_table') {
          await DB_GBKP.prepare(`DROP TABLE IF EXISTS ${tableName}`).run();
          await createTableIfNotExists(DB_GBKP, tableName);
        ...
        return await deleteContact(DB_GBKP, tableName, id, corsHeaders);
