...
// =========================================================
const CONFIG = {
  DB_BINDING : 'DB3',   // nama binding di Cloudflare Pages
  DB_NAME    : 'db3',   // nama database D1 (info saja)
};
// =========================================================
    ...
    switch (method) {
      ...
      case 'POST':
        if (action === 'bulk_insert') {
          return await bulkInsert(request, db, tableName, corsHeaders);
        }
...
// ── BULK INSERT: terima array rows, insert dalam 1 batch D1 ──
async function bulkInsert(request, db, tableName, corsHeaders) {
  ...
  // Pecah menjadi chunk 100 agar tidak melebihi batas D1 batch
  const CHUNK = 100;
  let totalInserted = 0;
