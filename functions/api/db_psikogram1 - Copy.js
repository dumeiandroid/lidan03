// functions/api/db_psikogram1.js
const API_VERSION = "v3.0-bulk_insert";
export async function onRequest(context) {
  const { request, env } = context;
  const { DB_PSIKOGRAM } = env;
  ...
  try {
    ...
    if (method === 'POST' && action === 'login') {
      ...
      await createTableIfNotExists(DB_PSIKOGRAM, targetTable);

      const user = await DB_PSIKOGRAM.prepare(
    ...
    if (method === 'GET' && action === 'list_tables') {
      const { results } = await DB_PSIKOGRAM.prepare(
    ...
    if ((method === 'POST' || method === 'GET') && action !== 'login' && action !== 'list_tables') {
      await createTableIfNotExists(DB_PSIKOGRAM, tableName);
    }

    switch (method) {
      case 'GET':
        if (id) {
          return await getContactById(DB_PSIKOGRAM, tableName, id, corsHeaders);
        }
        return await getFilteredContacts(request, DB_PSIKOGRAM, tableName, corsHeaders);

      case 'POST':
        return await createContact(request, DB_PSIKOGRAM, tableName, corsHeaders);

      case 'PUT':
        if (!id) throw new Error('id_x required for PUT.');
        return await updateContact(request, DB_PSIKOGRAM, tableName, id, corsHeaders);

      case 'DELETE':
        // --- DROP TABLE (reset id_x) ---
        if (action === 'drop_table') {
          await DB_PSIKOGRAM.prepare(`DROP TABLE IF EXISTS ${tableName}`).run();
          await createTableIfNotExists(DB_PSIKOGRAM, tableName);
        ...
        return await deleteContact(DB_PSIKOGRAM, tableName, id, corsHeaders);
