...
export async function onRequest(context) {
  ...
  try {
    if (method === 'GET') {
      return await handleGetSingle(env.DB_LIDAN_CO_ID, tableName, id, corsHeaders);
    } else if (method === 'PUT') {
      return await handleUpdate(request, env.DB_LIDAN_CO_ID, tableName, id, corsHeaders);
    } else if (method === 'DELETE') {
      return await handleDelete(env.DB_LIDAN_CO_ID, tableName, id, corsHeaders);
    } else {
...
async function handleGetSingle(DB_LIDAN_CO_ID, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const result = await DB_LIDAN_CO_ID.prepare(query).bind(id).first();
...
async function handleUpdate(request, DB_LIDAN_CO_ID, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const existsResult = await DB_LIDAN_CO_ID.prepare(existsQuery).bind(id).first();
    ...
    const updateResult = await DB_LIDAN_CO_ID.prepare(updateQuery).bind(...updateValues).run();
...
async function handleDelete(DB_LIDAN_CO_ID, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const deleteResult = await DB_LIDAN_CO_ID.prepare(deleteQuery).bind(id).run();
